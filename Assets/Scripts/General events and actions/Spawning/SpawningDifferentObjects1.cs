...
public class SpawningDifferentObjects1 : MonoBehaviour, IState
{
    [SerializeField]
     ...
     protected virtual Vector3 SpawnPoint()
    {
        ...
        //Высота постоянна
        offset.y = baseParameters.areaSpawning.y;

        offset.x = baseParameters.areaSpawning.x * UnityEngine.Random.Range(-.5f, .5f);
        offset.z = baseParameters.areaSpawning.z * UnityEngine.Random.Range(-.5f, .5f);

        return offset + spawnerLocation.transform.position;
    }
}
